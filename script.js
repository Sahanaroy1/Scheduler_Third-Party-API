...
$(document).ready(function () {
  $(".saveBtn").on("click", function () {
    ...
    var time = $(this).parent().attr("local-storage-key");
...
function generateTimeSlot() {
  for (var hour = time.startHour; hour <= time.endHour; hour++) {
    ...
    var html = ` <div id="hour-${hour}" class="row time-block past" local-storage-key=${hour}>
...
function updateTimeSlot() {
  ...
  $(".time-block").each(function (index, element) {
    var hour = $(element).attr("local-storage-key");
